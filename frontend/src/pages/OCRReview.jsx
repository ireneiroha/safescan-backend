...
export default function OCRReview() {
    ...
    const [text, setText] = useState(location.state?.extractedText ?? '')

    const handleContinue = () => {
        if (!text.trim()) return
        navigate('/confirm', { state: { extractedText: text, scanId, productCategory } })
    }
    ...
    return (
        <div className="mx-auto max-w-md md:max-w-[1440px] px-4 py-8 md:px-10 md:py-12">
            <div className="md:max-w-2xl md:mx-auto">
                ...
                <div className="mb-6">
                    <h1 className="text-2xl font-bold text-text-title">Review Scanned Text</h1>
                    <p className="text-sm text-text-secondary mt-1">
                        This is the raw text extracted from your image. Correct any errors before continuing.
                    </p>
                </div>

                <div className="mb-2">
                    <label className="block text-sm font-bold text-text-title mb-2">Extracted Text</label>
                    <textarea
                        value={text}
                        onChange={(e) => setText(e.target.value)}
                        placeholder="No text was extracted. You can type the ingredients manually."
                        rows={12}
                    ...
                    <p className="text-xs text-text-secondary mt-1.5">
                        Fix any OCR errors, then tap Continue to extract the ingredient list.
                    </p>
                </div>
